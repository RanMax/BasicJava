 ...
 * jdbc_ex. As requests are made of the sorter (like getValueAt(row, col))
 * they are passed to the underlying jdbc_ex after the row numbers
 * have been translated via the internal mapping array. This way,
 * the TableSorter appears to hold another copy of the table
 * with the rows in a different order.
 * <p/>
 * TableSorter registers itself as a listener to the underlying jdbc_ex,
 * just as the JTable itself would. Events recieved from the jdbc_ex
 * are examined, sometimes manipulated (typically widened), and then
 * passed on to the TableSorter's listeners (typically the JTable).
 * If a change to the jdbc_ex has invalidated the order of TableSorter's
